    ...
    def wrapper(*args, **kwargs):
        ...
        if guts.user.is_superuser or guts.user == project.admin:
            ...
            return ForbiddenResponse(
                "You must be an administrator for the %s project." % project.title
            )
...
def project_agreement(guts, project):
    ...
    return TemplateResponse(
        template, {'counts': same_counts, 'task_count': tasks.count()}
    )
        ...
        if action == "Upload":
            ...
            if item.is_valid():
                ...
                if not hasattr(settings, "PROCESS_INLINE") or settings.PROCESS_INLINE:
                    ...
                    pu.save()
                    message = (
                        "Upload complete to project %s, tasks processed" % project.id
                    )
                else:
            ...
            return ErrorResponse(
                "Bad form", "Action parameter %s not understood" % action
            )
...
def project_stats(guts, project):
    user_response_counts = (
        Response.objects.filter(task__project=project)
        .values("user__username")
        .annotate(Count("id"))
    )
    template = get_template("project/stats.html")
    return TemplateResponse(
        template, {'user_response_counts': user_response_counts, 'project': project}
    )
